...
function getVisualHTML(obj) {
  ...
  if (obj.type === 'dwarf') return '<div style="width:60px;height:60px;border-radius:50%;background:radial-gradient(circle at 30% 30%, ' + lightenColor(objColor, 30) + ', ' + objColor + ');box-shadow:0 0 20px ' + objColor + ';"></div>';
  if (obj.type === 'galaxy') return '<svg width="80" height="80" viewBox="0 0 80 80"><path d="M40,20 Q30,10 40,0 T40,20 Q50,30 40,40 T40,20 Q30,50 40,60 T40,20 Q50,70 40,80 T40,20 Q70,90 80,100" stroke="' + lightenColor(objColor, 20) + '" stroke-width="3" fill="none" opacity="0.7"/></svg>';
  if (obj.type === 'blackhole') return '<div style="position:relative;width:80px;height:80px;"><div style="width:40px;height:40px;background:#000;border-radius:50%;position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);"></div><div style="width:70px;height:70px;border:2px dashed ' + lightenColor(objColor, 30) + ';border-radius:50%;position:absolute;top:50%;left:50%;transform:translate(-50%,-50%) rotateX(75deg);animation:accretionSpin 3s linear infinite;"></div></div>';
  if (obj.type === 'constellation') return '<div style="width:100px;height:100px;border-radius:50%;background:radial-gradient(circle at 30% 30%, ' + lightenColor(objColor, 30) + ', ' + objColor + ');box-shadow:0 0 20px ' + objColor + ';"></div>';
  return '<div style="width:60px;height:60px;border-radius:50%;background:' + objColor + ';"></div>';
}
...
function createConstellationSVG(obj) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (s1 && s2) svg += '<line x1="' + (s1.x * 3) + '" y1="' + (s1.y * 3) + '" x2="' + (s2.x * 3) + '" y2="' + (s2.y * 3) + '" stroke="rgba(255,255,255,0.4)" stroke-width="2" stroke-linecap="round"/>';
  }
  for (let i = 0; i < stars.length; i++) {
    const s = stars[i], x = s.x * 4, y = s.y * 4, r = s.size * 1.8;
    svg += '<circle cx="' + x + '" cy="' + y + '" r="' + r + '" fill="' + s.color + '" filter="url(#starGlow)"/>';
    svg += '<text x="' + x + '" y="' + (y - r - 6) + '" fill="rgba(255,255,255,0.9)" font-size="11" text-anchor="middle" font-family="Comfortaa,cursive">' + s.name + '</text>';
  }
  svg += '<defs><filter id="starGlow"><feGaussianBlur stdDeviation="2" result="blur"/><feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs></svg>';
  return svg;
}
